...
/**
* Contains a hash map of {@link DGMNode} objects indexed by the
* fragment ID stored in the depictor of each node.  It is a basic
...
public class DGMHashMap extends HashMap implements Externalizable {
	...
	*/
	public void putDGM(DGMNode in) {
		put(in.getMyDraw().getFragID(), in);
		if (firstNode == null)
			firstNode = in;
	}

	/**
	* Gets the first node added to the hash map.
	*/
	public DGMNode getFirstNode() {
		return (firstNode);
	}
	...
	*/
	public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
		try {
			...
			int sz = v.length;
			int count;
			firstNode = (DGMNode) (v[0]);
			VersionBuffer.chkNul(firstNode);
			for (count = 1; count < sz; ++count) {
				DGMNode dgm = (DGMNode) (v[count]);
				VersionBuffer.chkNul(dgm);
				putDGM(dgm);
			}
		}
	...
	/**
	* Holds a reference to the first DGMNode stored for this depictor.
	*/
	protected DGMNode firstNode = null;
};
