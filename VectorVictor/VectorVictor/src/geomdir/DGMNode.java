...
/**
* A node that contains a depictor and its coordinate context.
* It is assumed that the coordinate context stored in the node
* was created from the node's depictor using the makeCoordContext
* method.  When the "draw" or "update" methods of the depictor are called,
* the rendering algorithm simply takes the coordinate context from each DGMNode
* and passes it to that node's depictor's "draw" or "update" method.  This node type
* is sometimes used instead of the {@link DrawObj} depictor type in some operations
