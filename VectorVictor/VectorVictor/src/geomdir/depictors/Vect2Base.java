...
*/
public abstract class Vect2Base<T extends Vect2Base, Q extends DefContext, R extends APPRec<Vect2Base.Vect2Evt,?,Q>> extends DrawObj<T,Q,R> implements Externalizable {
	final static int VectPort = 1;
	final static int HDPort = 2;
	final static int PrevPort = 100;

	protected static enum Vect2Evt {
	...
	R checkEraseControls(
		...
		if ((((hDGetMovable()).value & DepictorPort.MABLE_ASGN_MASK) > 0)) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Vect2Evt.MANUAL_DRAG_VECT_POS);
		...
		if ((((getMovable()).value & DepictorPort.MABLE_ASGN_MASK) > 0)) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Vect2Evt.MANUAL_DRAG_VECT_DIS);
				NewRec.clickPriority = Priority;
				ret = NewRec;
				LastClick = DepictorPort.MatchResult.MATCH;
			}
		...
		if (noSymBindings()) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Vect2Evt.MANUAL_DRAG_VEC_REAL);
				NewRec.clickPriority = Priority;
				ret = NewRec;
				LastClick = DepictorPort.MatchResult.MATCH;
			}
	...
	*/
	public void delVar(DepictorPort ThePort, R in, FlexString CurString) {
		...
		R MyRec = in;
		if (MyRec.getValue() == Vect2Evt.MANUAL_DRAG_VECT_POS) {
		...
		if (MyRec.getValue() == Vect2Evt.MANUAL_DRAG_VECT_DIS) {
			Object[] Form = { this };
			ThePort.insertFormattedString(Form, CurString);
		...
		if (MyRec.getValue() == Vect2Evt.MANUAL_DRAG_VEC_REAL) {
	...
	*/
	...
	static transient Vect2Base UndoNode = null;
	...
	*/
	public void singleClickControl(
		...
		if ((toolMode == DepictorPort.ToolMode.GEO_PAD_MODE)) {
			...
			if (MRec.getValue() == Vect2Evt.MANUAL_DRAG_VECT_DIS) {
	...
	*/
	protected void dragTextEvent(
		...
		PointF InPt) {
		R InRec = in;
		Vect2Evt VectDragMode = InRec.getValue();
	...
	*/
	...
	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Vect2Base.class));
	}
