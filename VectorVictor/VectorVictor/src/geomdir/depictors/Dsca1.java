...
*/
public class Dsca1 extends DrawObj implements Externalizable {
	...
	public void datCpy(Dsca1 out) {
		drDatCpy(out);
		/* out.HdPoint = HdPoint;
		out.TlPoint = TlPoint; */
	};
	public Meta copyNode() {
		Dsca1 temp = new Dsca1();
		datCpy(temp);
		return (temp);
	};
	public Meta copySub() {
	...
	*/
	private final void updateYourselfTypeA(DepictorPort ThePort, CoordContext PrtCon, boolean bound, int ToolMode) {
		...
		if (ToolMode == DepictorPort.ErasureMode) {
			Mvec t1 = new Mvec();
			Mvec t2 = new Mvec();
			double oneT = 1.0 / 3.0;
			double twoT = 2.0 / 3.0;
	...
	*/
	private final void updateYourselfTypeB(DepictorPort ThePort, CoordContext PrtCon, boolean bound, int ToolMode) {
		DefContext Dcon = (DefContext) PrtCon;
		if (DragThrough) {
			tLGetHex(Dcon).getGlo().setBasis1((hDGetVect()).getBasis1() - dir.getBasis1() * getVect().getBasis());
			tLGetHex(Dcon).getGlo().setBasis2((hDGetVect()).getBasis2() - dir.getBasis2() * getVect().getBasis());
...
//	};
	...
	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Dsca1.class));
	}
