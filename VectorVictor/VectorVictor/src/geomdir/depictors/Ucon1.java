...
* "vector port".
*/
public class Ucon1 extends DrawObj implements Externalizable {
	...
	public void datCpy(Ucon1 out) {
		drDatCpy(out);
		/* out.HdPoint = HdPoint;
		out.TlPoint = TlPoint; */
	};
	public Meta copyNode() {
		Ucon1 temp = new Ucon1();
		datCpy(temp);
		return (temp);
	};
	public Meta copySub() {
		...
		if (((hDGetMovable()).value >= DepictorPort.MABLE_BY_ANY) && (!bound)) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == NoMatch)) {
				if (getUseMaintainUnit()) {
					...
					NewRec.setTransCoord((CoordContext) (Dcon.copyAll()));
					HDHexGlo.copyAllInfo(hDGetHex((DefContext) (NewRec.getTransCoord())).getGlo());
					NewRec.setValue(ManualDragVectPos);
	...
	*/
	...
	public PointF hDGetPoint(DefContext in) {
		return (hDGetHex(in).getLoc());
	};
	public PointF tLGetPoint(DefContext in) {
		return (tLGetHex(in).getLoc());
	};

	static transient Ucon1 UndoNode = null;
	...
	*/
	public void dragDisplayControl(
		...
		switch (ToolMode) {
			...
			case DepictorPort.FreeTransformationMode :
				updateUnitValue(ThePort, Dcon, bound, (APPRec) in, InPt, 2.0);
				break;

			case DepictorPort.AccessoryTransMode :
				updateUnitValueAcTrans(ThePort, Dcon, bound, (APPRec) in, InPt, 1.0);
				break;
			...
			case DepictorPort.GeoPadMode :
				updateUnitValue(ThePort, Dcon, bound, (APPRec) in, InPt, 1.0);
				break;
	...
	protected void dragDilateEvent(
		...
		switch (VectDragMode) {
			case ManualDragVectPos :
				updateUnitValueBackDilate(ThePort, Dcon, bound, (APPRec) in, InPt, 1.5);
				break;

			case ManualDragVectDis :
				updateUnitValue(ThePort, Dcon, bound, (APPRec) in, InPt, 1.5);
				break;
		}
	...
	*/
	protected void updateUnitValueBackDilate(
		...
		double Mult) {
		DefContext Dcon = (DefContext) PrtCon;
		Hexar OTLHex = tLGetHex((DefContext) (InRec.getTransCoord()));
	...
	*/
	...
	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Ucon1.class));
	}
...
/*	public static class UconCustControl extends Object implements VerdantiumPropertiesEditor {
		...
		public UconCustControl(Ucon1 in, ToolPort port, XKit MyAp) {
			super();
			target = in;
			ThePort = port;
			MyPan = new JPanel();
