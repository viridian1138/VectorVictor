...
/**
* Depicts a scalar comparison between two elements.
* The "_md_" port of the depictor defines the spatial position
* of the comparator.
* For more information on depictors in general see {@link geomdir.DrawObj}.
*/
public abstract class Compar1Base<T extends Compar1Base, Q extends DefContext, R extends APPRec<Compar1Base.Compar1Evt,?,Q>> extends DrawObj<T,Q,R> implements Externalizable {
	final static int VectPort = 1;
	final static int MDPort = 3;
	final static int PrevPort = 100;

	protected static enum Compar1Evt {
	...
	*/
	public void updateYourself(DepictorPort ThePort, Q PrtCon, boolean bound, DepictorPort.ToolMode toolMode) {
		...
		MyRect.set(mDGetHex(Dcon).getLoc().x - 16, mDGetHex(Dcon).getLoc().y - 32, 
				mDGetHex(Dcon).getLoc().x + 16, mDGetHex(Dcon).getLoc().y );
	...
	*/
	public void drawYourself(DepictorPort ThePort, Q PrtCon, boolean bound, Canvas g, Paint p, DepictorPort.ToolMode toolMode) {
		...
		if ((FrontLineVisible) || (toolMode == DepictorPort.ToolMode.COLOR_MODE)) {
			double MyVal = getVect().getBasis();
			int MyStat = 0;
			if (MyVal > 0.001)
				MyStat = 1;
			else if (MyVal < -0.001)
			...
			Integer c1 = new Integer( Color.WHITE );
			if (MyStat == 1)
				c1 = Win.getCyanIndex();
			if (MyStat == -1)
				c1 = Win.getMagentaIndex();
			...
			if (MyStat == 1) {
				g.drawLine(
					mDGetHex(Dcon).getLoc().x - 12,
					mDGetHex(Dcon).getLoc().y - 28,
					mDGetHex(Dcon).getLoc().x + 12,
					mDGetHex(Dcon).getLoc().y - 16, p);

				g.drawLine(
					mDGetHex(Dcon).getLoc().x + 12,
					mDGetHex(Dcon).getLoc().y - 16,
					mDGetHex(Dcon).getLoc().x - 12,
					mDGetHex(Dcon).getLoc().y - 4, p);
			}
			else if (MyStat == -1) {
				g.drawLine(
					mDGetHex(Dcon).getLoc().x + 12,
				...
				g.drawLine(
					mDGetHex(Dcon).getLoc().x - 12,
					mDGetHex(Dcon).getLoc().y - 20,
					mDGetHex(Dcon).getLoc().x + 12,
					mDGetHex(Dcon).getLoc().y - 20, p);

				g.drawLine(
					mDGetHex(Dcon).getLoc().x - 12,
					mDGetHex(Dcon).getLoc().y - 12,
					mDGetHex(Dcon).getLoc().x + 12,
					mDGetHex(Dcon).getLoc().y - 12, p);
			}
		}
	...
	*/
	protected void drawEraseTools(DepictorPort ThePort, Q Dcon, boolean bound, Canvas g, Paint p, DepictorPort.ToolMode toolMode) {
		if (((getMovable()).value & DepictorPort.MABLE_ASGN_MASK) > 0) {
			...
			ThePort.paintOrangeKnob(g, p, mDGetPoint(Dcon).x, mDGetPoint(Dcon).y - 16);
	...
	R checkEraseControls(
		...
		if ((((getMovable()).value & DepictorPort.MABLE_ASGN_MASK) > 0)) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Compar1Evt.MANUAL_DRAG_VECT_DIS);
		...
		if ((((mDGetMovable()).value & DepictorPort.MABLE_ASGN_MASK) > 0)) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Compar1Evt.MANUAL_DRAG_MD_POS);
				NewRec.clickPriority = Priority;
				ret = NewRec;
				LastClick = DepictorPort.MatchResult.MATCH;
			}
		...
		if (noSymBindings()) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Compar1Evt.MANUAL_DRAG_VEC_REAL);
				NewRec.clickPriority = Priority;
				ret = NewRec;
				LastClick = DepictorPort.MatchResult.MATCH;
			}
	...
	*/
	public void delVar(DepictorPort ThePort, R in, FlexString CurString) {
		...
		if (MyRec.getValue() == Compar1Evt.MANUAL_DRAG_MD_POS) {
		...
		if (MyRec.getValue() == Compar1Evt.MANUAL_DRAG_VECT_DIS) {
			Object[] Form = { this };
			ThePort.insertFormattedString(Form, CurString);
		...
		if (MyRec.getValue() == Compar1Evt.MANUAL_DRAG_VEC_REAL) {
	...
	static Compar1Base UndoNode = null;
	...
	*/
	...
	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Compar1Base.class));
	}
