...
/**
* Depicts the phasor-valued part of a multivector using an angular depiction in the alternate sense.
...
*/
public abstract class Angle2Base<T extends Angle2Base, Q extends DefContext, R extends APPRec<Angle2.Angle2Evt,?,Q>> extends DrawObj<T,Q,R> implements Externalizable {
	final static int VectPort = 1;
	final static int HDPort = 2;
	final static int MDPort = 3;
	final static int PrevPort = 100;

	protected static enum Angle2Evt {
	...
	*/
	protected void drawPolarSegment(DepictorPort ThePort, Q Dcon, boolean bound, Canvas g, Paint p, DepictorPort.ToolMode toolMode) {
	...
	*/
	void drawFwdMidArrow(DepictorPort ThePort, Canvas g, Paint p, Q Dcon, double RadLen, double MyAng) {
	...
	*/
	void drawFwdEndArrow(DepictorPort ThePort, Canvas g, Paint p, Q Dcon, double RadLen, double MyAng) {
		double tx = mLGetHex(Dcon).getPtx() - hDGetHex(Dcon).getPtx();
		double ty = mLGetHex(Dcon).getPty() - hDGetHex(Dcon).getPty();
		boolean tmp = FrontLineVisible;
		double xe1 = mLGetHex(Dcon).getPtx();
		double ye1 = mLGetHex(Dcon).getPty();
		double xe2 = mLGetHex(Dcon).getPtx();
		double ye2 = mLGetHex(Dcon).getPty();
	...
	*/
	...
	/**
	* Returns a R iff. the user clicked in the gravity field of a depictor.
	*/
	...
	R checkEraseControls(
		...
		if ((((hDGetMovable()).value & DepictorPort.MABLE_ASGN_MASK) > 0)) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Angle2Evt.MANUAL_DRAG_VECT_POS);
		...
		if ((((getMovable()).value & DepictorPort.MABLE_ASGN_MASK) > 0)) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Angle2Evt.MANUAL_DRAG_VECT_DIS);
				NewRec.clickPriority = Priority;
				ret = NewRec;
				LastClick = DepictorPort.MatchResult.MATCH;
			}
		...
		if ((((mDGetMovable()).value & DepictorPort.MABLE_ASGN_MASK) > 0)) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Angle2Evt.MANUAL_DRAG_MD_POS);
				NewRec.clickPriority = Priority;
				ret = NewRec;
				LastClick = DepictorPort.MatchResult.MATCH;
			}
		...
		if (noSymBindings()) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				NewRec.setValue(Angle2Evt.MANUAL_DRAG_VEC_REAL);
				NewRec.clickPriority = Priority;
				ret = NewRec;
				LastClick = DepictorPort.MatchResult.MATCH;
			}
	...
	*/
	public void delVar(DepictorPort ThePort, R in, FlexString CurString) {
		...
		R MyRec = in;
		if (MyRec.getValue() == Angle2Evt.MANUAL_DRAG_VECT_POS) {
		...
		if (MyRec.getValue() == Angle2Evt.MANUAL_DRAG_MD_POS) {
		...
		if (MyRec.getValue() == Angle2Evt.MANUAL_DRAG_VECT_DIS) {
			Object[] Form = { this };
			ThePort.insertFormattedString(Form, CurString);
		...
		if (MyRec.getValue() == Angle2Evt.MANUAL_DRAG_VEC_REAL) {
	...
	static Angle2Base UndoNode = null;
	...
	*/
	public void singleClickControl(
		...
		if ((toolMode == DepictorPort.ToolMode.GEO_PAD_MODE)) {
			...
			if (MRec.getValue() == Angle2Evt.MANUAL_DRAG_VECT_DIS) {
	...
	*/
	...
	public Hexar mLGetHex(Q in) {
		return (in.getHex8());
	};
	
	public final double getStrtAng(Q in) {
		return (in.getD1());
	}
	public final void setStrtAng(Q in, double val) {
		in.setD1(val);
	}
	public final double getEndAng(Q in) {
		return (in.getD2());
	}
	public final void setEndAng(Q in, double val) {
		in.setD2(val);
	}
	...
	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Angle2Base.class));
	}
...
//		return ((new Angle2BaseCustControl(this, ThePort, InAp)));
//	};
	...
	*/
/*	public static class Angle2BaseCustControl extends Object implements VerdantiumPropertiesEditor {
		Angle2Base target;
		DepictorPort ThePort;
		...
		public Angle2BaseCustControl(Angle2Base in, ToolPort port, XKit MyAp) {
			super();
			target = in;
			ThePort = port;
			MyPan = new JPanel();
