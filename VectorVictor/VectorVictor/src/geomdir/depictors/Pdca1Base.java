...
public abstract class Pdca1Base<T extends Pdca1Base, Q extends DefContext, R extends APPRec<Pdca1Base.Pdca1Evt,?,Q>> extends DrawObj<T,Q,R> implements Externalizable {
	...
	*/
	private final void drawEraseTools(
		...
		boolean bound,
		Canvas g, Paint p,
		DepictorPort.ToolMode toolMode) { // ###########################################################################3
		//		if( ( ( hDGetMovable() ).value & DepictorPort.MableAsgnMask ) > 0 )
		//			{
	...
	*/
	private final R checkEraseControls(
		DepictorPort ThePort,
		Q Dcon,
		boolean bound,
		PointF InPt,
	...
	*/
	private final R checkTransControls(
		DepictorPort ThePort,
		Q Dcon,
		boolean bound,
		PointF InPt,
	...
	*/
	private final R checkColorControls(
		DepictorPort ThePort,
		Q Dcon,
		boolean bound,
		PointF InPt,
	...
	*/
	private final R checkRotateControls(
		DepictorPort ThePort,
		Q Dcon,
		boolean bound,
		PointF InPt,
	...
	* Creates a DynRunner that rotates the depictor around its base.
	*/
	private final DynRunner createObFwdRotateDyn(DepictorPort ThePort, Q Dcon, boolean bound, boolean Pos) {
		DynRunner Dyn = ThePort.createDynRunner(bound, HDHexGlo);
		Object[] LHS = { "_d1", this };
		Object[] RHS = { " ", " ( UnitOf( '~glo' \\- ", "_hd", this, " ) ) " };
	...
	*/
	private final R checkDilateControls(
		DepictorPort ThePort,
		Q Dcon,
		boolean bound,
		PointF InPt,
	...
	*/
	private final R checkAccControls(
		DepictorPort ThePort,
		Q Dcon,
		boolean bound,
		PointF InPt,
	...
	*/
	private final R checkGeoPadControls(
		DepictorPort ThePort,
		Q Dcon,
		boolean bound,
		PointF InPt,
	...
	*/
	...
	public PointF dXGetPoint(Q Dcon) {
		return (dXGetHex(Dcon).getLoc());
	};

	static Pdca1Base UndoNode = null;
	...
	*/
	...
	public Hexar dXGetHex(Q in) {
		return (in.getHex6());
	};
	public Hexar temp3GetHex(Q in) {
		return (in.getHex7());
	};
	public Hexar temp4GetHex(Q in) {
		return (in.getHex8());
	};
	...
	*/
	...
	//		{ Pdca1BaseHelp.run( in ); } !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1

	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Pdca1Base.class));
	}
