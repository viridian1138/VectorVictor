...
import geomdir.depictors.Ruler1Base.RulerContextImpl;
...
/**
* Depicts a real axis.  This measures a distance in terms of a unit defined by dragging.
* The "vector port"
...
public abstract class Rlax1Base<T extends Rlax1Base, Q extends Rlax1Base.RlaxContext, R extends APPRec<?,Q>> extends DrawObj<T,Q,R> implements Externalizable {
	...
	public static class RlaxContextImpl extends RlaxContext<RlaxContextImpl>
	{
		public RlaxContextImpl(int MajorGraduationLevel, int MinorGraduationDelta)
		{
			super(MajorGraduationLevel, MinorGraduationDelta);
		}
		
		@Override
		public RlaxContextImpl makeMeta() {
			return( new RlaxContextImpl( MajGradLev , MinGradDelta ) );
		}
	...
	*/
	public R clickedInRegion(
		...
		PointF LocEvent,
		DepictorPort.ToolMode toolMode) {
		R MyRec = clickedInRegionComp(ThePort, PrtCon, bound, LocEvent, toolMode);
	...
	*/
	...
	static Rlax1Base UndoNode = null;
...
//		Rlax1BaseHelp.run(in);
//	}

	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Rlax1Base.class));
	}
