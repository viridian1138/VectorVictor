...
*/
public class Para1 extends DrawObj implements Externalizable {
	...
	*/
	DynRunner createTransDyn(DepictorPort ThePort, DefContext Dcon) {
	...
	*/
	DynRunner createShearDyn(DepictorPort ThePort, DefContext Dcon, boolean bound) {
	...
	*/
	ClickRec checkFreeTransControls(
		DepictorPort ThePort,
		DefContext Dcon,
		boolean bound,
		PointF InPt,
	...
	*/
	DynRunner createDeformDyn(DepictorPort ThePort, DefContext Dcon, boolean bound) {
		DynRunner Dyn = ThePort.createDynRunner(bound, HDHexGlo);
	...
	*/
	DynRunner createRotateDyn(DepictorPort ThePort, DefContext Dcon, boolean bound) {
		DynRunner Dyn = ThePort.createDynRunner(bound, HDHexGlo);
	...
	*/
	public void handleCoordAdjust(
		...
		if (((mXGetMovable()).value >= DepictorPort.MABLE_BY_ANY)) {
			Hexar std = mXGetHex((DefContext) Std);
			Hexar ini = mXGetHex((DefContext) Initial);
...
/*	public void saveProperties(DepictorPort ThePort, VTextProperties MyProp) {
	...
	*/
	...
	public Hexar paraPt3GetHex(DefContext in) {
		return (in.getHex6());
	};
	public Hexar paraPt4GetHex(DefContext in) {
		return (in.getHex7());
	};
	...
	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Para1.class));
	}
