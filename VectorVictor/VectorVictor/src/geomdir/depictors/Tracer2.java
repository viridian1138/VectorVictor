...
*    | 11/05/2002            | Thorn Green (viridian_1138@yahoo.com)           | Continuous graphing for GeoCard.                                     | Created Tracer2 using code from Tracer1.
...
/**
* Depicts a continuous graph.
* The position of the trace point is defined by the "vector port" of
* the depictor.  The independent var. of the graph is define in the "_lq" port.
* This class is not finished.
* For more information on depictors in general see {@link geomdir.DrawObj}.
*/
public class Tracer2 extends DrawObj implements Externalizable {
	...
	public void datCpy(Tracer2 out) {
		drDatCpy(out);
		/* out.HdPoint = HdPoint;
		out.TlPoint = TlPoint; */
	};
	public Meta copyNode() {
		Tracer2 temp = new Tracer2();
		datCpy(temp);
		return (temp);
	};
	public Meta copySub() {
	...
	*/
	public boolean noSymBindings() {
		...
		if ((((getSelfAlloc()).value & DepictorPort.SELF_ALLOC_DROPABLE) != 0)
			&& (((lQGetSelfAlloc()).value & DepictorPort.SELF_ALLOC_DROPABLE) != 0)) {
			NoBound = true;
		}
	...
	*/
	public void setValuePort(FlexString Prefix) {
		PortMode = -1;
		String VectPrefix = "";
		String LQPrefix = "_lq_";
		String PrPrefix = "_&_";

		if (Prefix.stcmp(VectPrefix) == 0)
			PortMode = VectPort;
		if (Prefix.stcmp(LQPrefix) == 0)
			PortMode = LQPort;
	...
	*/
	...
	public void handleGraphUpdate(DepictorPort ThePort) {
		if (/* ThePort.solverLinkageIsChaining( ) */
			true) {
			double a1 = -10.0;
			double a2 = 10.0;
			int max = 1000;
			int mmax = max - 1;
			if ((lQGetMovable().value) >= DepictorPort.MABLE_BY_DIFFERENTIABLE) {
				int count;
				Mvec tmp = new Mvec();
				lQGetVect().mcpy(tmp);
				Mvec pt = new Mvec();
				...
				for (count = 0; count < max; ++count) {
					double val = a1 + (a2 - a1) * count / mmax;
					Tracer2Plugin.setReturnValue(val);
					Dyn.dragExecute(ThePort, pt);
					Mvec nmvec = new Mvec();
					VectGlo.mcpy(nmvec);
					PointList.insertRight(nmvec);
				}

				if (max > 0) {
					Tracer2Plugin.setReturnValue(tmp.getBasis());
					Dyn.dragExecute(ThePort, pt);
				}
	...
	*/
	protected void drawTextTools(DepictorPort ThePort, TracerContext Dcon, boolean bound, Canvas g, Paint p, DepictorPort.ToolMode toolMode) {
	...
	*/
	...
	public Tracer2() {
		...
		LQSelfAlloc.value = DepictorPort.SELF_ALLOC_DROPABLE + DepictorPort.SELF_ALLOC_ENGINE_IGNORE;
		VectGlo = new Mvec();
		LQPtr = new Mvec();
		Movable.value = DepictorPort.MABLE_BY_ANY;
		LQMovable.value = DepictorPort.MABLE_BY_ANY;
		VectName = new FlexString();
	};

	static Tracer2 UndoNode = null;
	...
	*/
	public void pushDragUndo() {
		UndoNode = new Tracer2();
		VectGlo.copyAllInfo(UndoNode.VectGlo);
		LQPtr.copyAllInfo(UndoNode.LQPtr);
	...
	*/
	public void loadProperties(DepictorPort ThePort, VTextProperties MyProp) throws DataFormatException {
		...
		ThePort.loadMvecProperties(MyProp, key + ".LQport", LQPtr);
	...
	*/
	public void saveProperties(DepictorPort ThePort, VTextProperties MyProp) {
		...
		ThePort.saveMvecProperties(MyProp, key + ".LQport", LQPtr);
	...
	*/
	...
	public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
		try {
			...
			LQVectName = (FlexString) (myv.getProperty("LQVectName"));
			LQMovable = (IntObj) (myv.getProperty("LQMovable"));
			VersionBuffer.chkNul(LQMovable);
			LQPtr = (Mvec) (myv.getProperty("LQPtr"));
	...
	*/
	public void writeExternal(ObjectOutput out) throws IOException {
		...
		if (LQVectName != null)
			myv.setProperty("LQVectName", LQVectName);
		myv.setProperty("LQMovable", LQMovable);
		myv.setProperty("LQPtr", LQPtr);
		myv.setProperty("LQSelfAlloc", LQSelfAlloc);
	...
	*/
	...
	private FlexString lQGetVectName() {
		return (LQVectName);
	};
	private IntObj lQGetMovable() {
		return (LQMovable);
	};
	private void lQSetMovable(IntObj in) {
		LQMovable = in;
	};
	private Mvec lQGetVect() {
		return (LQPtr);
	};
	private IntObj lQGetSelfAlloc() {
		return (LQSelfAlloc);
	};
	...
	*/
	...
	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Tracer2.class));
	}
	...
	FlexString LQVectName;
	IntObj LQMovable = new IntObj();
	protected Mvec LQPtr = new Mvec();
	IntObj LQSelfAlloc = new IntObj();
};
