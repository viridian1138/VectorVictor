...
/**
* Depicts a bargraph representation of the phasor-valued part of a multivector.
...
public abstract class Bar1Base<T extends Bar1Base, Q extends DefContext, R extends APPRec<Bar1Base.Bar1Evt,?,Q>> extends DrawObj<T,Q,R> implements Externalizable {
	...
	*/
	public void drawYourself(DepictorPort ThePort, Q PrtCon, boolean bound, Canvas g, Paint p, DepictorPort.ToolMode toolMode) {
		...
		Q Dcon = PrtCon;
		drawBarPhasor(Win, mDGetHex(Dcon), g, p, getVect(), toolMode, ArcToolRad);
				...
				case ACCESSORY_TRANS_MODE :
					{
						...
						if ((getMovable().value) >= DepictorPort.MABLE_BY_DIFFERENTIABLE) {
							...
							double xw = getVect().getBasis() * ThePort.getRealBar().value;

							ThePort.paintBlueKnob(g, p, mDGetPoint(Dcon).x - ArcToolRad, mDGetPoint(Dcon).y - xw);

							xw = getVect().getBasis12() * ThePort.getImBar().value;
	...
	*/
	protected void drawEraseTools(DepictorPort ThePort, Q Dcon, boolean bound, Canvas g, Paint p, DepictorPort.ToolMode toolMode) {
		if (((getMovable()).value & DepictorPort.MABLE_ASGN_MASK) > 0) {
			...
			ThePort.paintOrangeKnob(g, p, mDGetPoint(Dcon).x - ArcToolRad, mDGetPoint(Dcon).y - xw);
	...
	R checkControls(DepictorPort ThePort, Q Dcon, boolean bound, PointF InPt, DepictorPort.ToolMode toolMode) {
		...
		if ((getMovable()).value >= DepictorPort.MABLE_BY_DIFFERENTIABLE) {
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				...
				tmp1 = new Mvec();
				tmp1.setBasis2(-1.0 / ThePort.getRealBar().value);
			...
			if ((Priority <= ClickRec.MIN_PRIORITY) && (LastClick == DepictorPort.MatchResult.NO_MATCH)) {
				...
				tmp1 = new Mvec();
				tmp1.setBasis2(-1.0 / ThePort.getImBar().value);
	...
	*/
	public double shapeGravityField(DepictorPort ThePort, Q Dcon, PointF in) {
		return (
			barPhasorGravityField(ThePort, in, mDGetHex(Dcon), getVect(), ArcToolRad, getBasicBackLineStrokeWidth()));
	}
	...
	*/
	...
	public static URL getUpImgUrl() {
		return (getDefaultUpImgUrl(Bar1Base.class));
	}
